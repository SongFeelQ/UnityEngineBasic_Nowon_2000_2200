...
public class CameraHandler : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        _boundingShapeXMin = _boundShape.transform.position.x + _boundShape.offset.x - _boundShape.size.x / 2f;
        _boundingShapeXMax = _boundShape.transform.position.x + _boundShape.offset.x + _boundShape.size.x / 2f;
        _boundingShapeYMin = _boundShape.transform.position.y + _boundShape.offset.y - _boundShape.size.y / 2f;
        _boundingShapeYMax = _boundShape.transform.position.y + _boundShape.offset.y + _boundShape.size.y / 2f;
    }
    ...
    private void Follow()
    {
        ...
        // X min bound
        if (smoothPos.x < _boundingShapeXMin + camWorldPosSize.x / 2f)
            smoothPos.x = _boundingShapeXMin + camWorldPosSize.x / 2f;
        // X max bound
        else if (smoothPos.x > _boundingShapeXMax - camWorldPosSize.x / 2f)
            smoothPos.x = _boundingShapeXMax - camWorldPosSize.x / 2f;
        // Y min bound
        if (smoothPos.y < _boundingShapeYMin + camWorldPosSize.y / 2f)
            smoothPos.y = _boundingShapeYMin + camWorldPosSize.y / 2f;
        else if (smoothPos.y > _boundingShapeYMax - camWorldPosSize.y / 2f)
            smoothPos.y = _boundingShapeYMax - camWorldPosSize.y / 2f;
